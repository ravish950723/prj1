...
def _classify_market_stage(df: pd.DataFrame) -> pd.Series:
    """
    Hedge-fund style stage classifier.
    Priority: Mark-Down > Distribution > Mark-Up > Accumulation > Neutral/Transition.
    Uses trend, slope, range compression, and distance from long-term means.
    """
    n = len(df)
    ...
    ema21 = pd.to_numeric(df.get("EMA_21", pd.Series(np.nan, index=df.index)), errors="coerce")
    ...
    adx = pd.to_numeric(df.get("ADX_14", pd.Series(np.nan, index=df.index)), errors="coerce").fillna(0.0)
    macd_hist = pd.to_numeric(df.get("MACD_hist", pd.Series(np.nan, index=df.index)), errors="coerce").fillna(0.0)
    macd_hist_slope = pd.to_numeric(df.get("MACD_hist_slope", pd.Series(np.nan, index=df.index)), errors="coerce").fillna(0.0)
    tight_range = pd.Series(df.get("tight_range", pd.Series(False, index=df.index)), index=df.index).fillna(False).astype(bool)
    near_support = pd.Series(df.get("near_support", pd.Series(False, index=df.index)), index=df.index).fillna(False).astype(bool)
    vol_surge = pd.to_numeric(df.get("VOL_SURGE_RATIO", pd.Series(np.nan, index=df.index)), errors="coerce").fillna(0.0)

    ema21_slope = ema21.diff().rolling(5, min_periods=3).mean().fillna(0.0)
    ema50_slope = ema50.diff().rolling(8, min_periods=5).mean().fillna(0.0)
    above_200 = close >= ema200
    below_200 = close < ema200

    markup = (close >= ema21) & (ema21 >= ema50) & (ema50 >= ema200) & (ema21_slope > 0) & (macd_hist >= 0)
    markdown = (close <= ema21) & (ema21 <= ema50) & (ema50 <= ema200) & (ema21_slope < 0) & (macd_hist <= 0)

    accumulation = (
        above_200
        & (ema50_slope >= -0.02 * close.fillna(0) / 100.0)
        & (
            tight_range
            | near_support
            | ((macd_hist_slope > 0) & (macd_hist > -0.15))
            | ((close >= ema21 * 0.985) & (ema21_slope > -0.01 * close.fillna(0) / 100.0))
        )
    )
    ...
    distribution = (
        ...
        & (
            tight_range
            | ((macd_hist_slope < 0) & (macd_hist <= 0.05))
            | ((vol_surge >= 1.2) & (close < ema21) & (adx >= 18))
        )
        & (~markup)
        & (~markdown)
    )
    ...
    stage.loc[markup] = "Mark-Up"
    stage.loc[markdown | below_200 & (ema21_slope < 0) & (macd_hist < 0)] = "Mark-Down"
...
def _clip01_series(s: pd.Series) -> pd.Series:
    return pd.to_numeric(s, errors="coerce").clip(0.0, 1.0).fillna(0.0)


def _add_orderflow_institutional_layer(df: pd.DataFrame) -> pd.DataFrame:
    """
    Add order-flow and institutional proxy features using OHLCV only.
    This gives the scoring layer a usable microstructure approximation even when
    true IBKR market depth is unavailable.
    """
    ...
    money_flow_mult = ((close - low) - (high - close)) / (rng + 1e-9)
    money_flow_mult = money_flow_mult.clip(-1.0, 1.0).fillna(0.0)
    signed_volume = np.sign(close.diff().fillna(0.0)) * volume
    adl = (money_flow_mult * volume).cumsum()
    adl_delta = adl.diff().fillna(0.0)

    close_location = ((close - low) / (rng + 1e-9)).clip(0.0, 1.0).fillna(0.5)
    upper_close = (close_location >= 0.65).astype(float)
    lower_close = (close_location <= 0.35).astype(float)
    body_pct = ((close - open_).abs() / (rng + 1e-9)).clip(0.0, 1.0).fillna(0.0)
    lower_wick = ((np.minimum(open_, close) - low) / (rng + 1e-9)).clip(0.0, 1.0).fillna(0.0)
    upper_wick = ((high - np.maximum(open_, close)) / (rng + 1e-9)).clip(0.0, 1.0).fillna(0.0)

    # Daily tape-style proxies
    buy_pressure = _clip01_series(0.45 * close_location + 0.35 * ((ret1 > 0).astype(float)) + 0.20 * body_pct)
    sell_pressure = _clip01_series(0.45 * (1.0 - close_location) + 0.35 * ((ret1 < 0).astype(float)) + 0.20 * body_pct)
    accumulation_signature = _clip01_series(
        0.35 * close_location
        + 0.25 * (vol_ratio.clip(0, 3) / 3.0)
        + 0.20 * ((adl_delta > 0).astype(float))
        + 0.20 * lower_wick
    )
    distribution_signature = _clip01_series(
        0.35 * (1.0 - close_location)
        + 0.25 * (vol_ratio.clip(0, 3) / 3.0)
        + 0.20 * ((adl_delta < 0).astype(float))
        + 0.20 * upper_wick
    )
    absorption_score = _clip01_series(0.55 * lower_wick + 0.25 * upper_close + 0.20 * (vol_ratio.clip(0, 2) / 2.0))
    distribution_pressure = _clip01_series(0.55 * upper_wick + 0.25 * lower_close + 0.20 * (vol_ratio.clip(0, 2) / 2.0))

    cmf20 = ((money_flow_mult * volume).rolling(20, min_periods=5).sum() / (volume.rolling(20, min_periods=5).sum() + 1e-9))
    cmf20 = cmf20.clip(-1.0, 1.0).fillna(0.0)
    obv = pd.to_numeric(df.get("OBV"), errors="coerce")
    obv = obv.ffill().fillna(0.0)
    obv_impulse = (obv.diff(5) / (obv.abs().rolling(20, min_periods=5).mean() + 1e-9)).clip(-1.0, 1.0).fillna(0.0)

    order_flow_imbalance = (
        0.32 * money_flow_mult
        + 0.23 * cmf20
        + 0.20 * obv_impulse
        + 0.15 * (vol_ratio.clip(0, 3) / 3.0) * np.sign(ret1).fillna(0.0)
        + 0.10 * (buy_pressure - sell_pressure)
    ).clip(-1.0, 1.0).fillna(0.0)

    order_flow_score = _clip01_series((order_flow_imbalance + 1.0) / 2.0)

    # Institutional participation proxies
    dollar_volume = (close * volume).fillna(0.0)
    dollar_vol_z = ((dollar_volume - dollar_volume.rolling(20, min_periods=5).mean()) /
                    (dollar_volume.rolling(20, min_periods=5).std() + 1e-9)).clip(-3.0, 3.0).fillna(0.0)
    stealth_accumulation = _clip01_series(
        0.35 * (cmf20.clip(0, 1))
        + 0.25 * absorption_score
        + 0.20 * (vol_ratio.clip(0, 2) / 2.0)
        + 0.20 * ((close >= close.rolling(10, min_periods=3).median()).astype(float))
    )
    stealth_distribution = _clip01_series(
        0.35 * ((-cmf20).clip(0, 1))
        + 0.25 * distribution_pressure
        + 0.20 * (vol_ratio.clip(0, 2) / 2.0)
        + 0.20 * ((close <= close.rolling(10, min_periods=3).median()).astype(float))
    )

    base_inst = pd.to_numeric(df.get("institutional_score"), errors="coerce").fillna(0.0)
    institutional_flow_score = _clip01_series(
        0.42 * base_inst
        + 0.18 * stealth_accumulation
        + 0.16 * (dollar_vol_z.clip(0, 3) / 3.0)
        + 0.14 * absorption_score
        + 0.10 * order_flow_score
        - 0.12 * stealth_distribution
    )

    # L2 proxy from bar structure
    spread_proxy_pct = ((rng / close.replace(0, np.nan)) * 100.0 * 0.18).clip(lower=0.01)
    spread_proxy_abs = (close * spread_proxy_pct / 100.0).fillna(0.0)
    best_bid_proxy = close - spread_proxy_abs / 2.0
    best_ask_proxy = close + spread_proxy_abs / 2.0
    microprice_proxy = close + order_flow_imbalance * spread_proxy_abs * 0.20

    liquidity_quality = np.where(
        (dollar_volume >= 5e8) & (spread_proxy_pct <= 0.08), "DEEP",
        np.where((dollar_volume >= 1e8) & (spread_proxy_pct <= 0.18), "GOOD", "THIN")
    )
    l2_status = np.where(np.isfinite(best_bid_proxy) & np.isfinite(best_ask_proxy), "PROXY", "UNAVAILABLE")

    df["Money Flow Multiplier"] = money_flow_mult
    df["CMF_20"] = cmf20
    df["ADL"] = adl
    df["ADL_DELTA"] = adl_delta
    df["Order Flow Imbalance"] = order_flow_imbalance
    df["Order Flow Score"] = order_flow_score
    df["Buy Pressure"] = buy_pressure
    df["Sell Pressure"] = sell_pressure
    df["Accumulation Signature"] = accumulation_signature
    df["Distribution Signature"] = distribution_signature
    df["Absorption Score"] = absorption_score
    df["Institutional Flow Score"] = institutional_flow_score
    df["Stealth Accumulation Score"] = stealth_accumulation
    df["Stealth Distribution Score"] = stealth_distribution
    df["Dollar Volume"] = dollar_volume
    df["Dollar Volume Z20"] = dollar_vol_z

    # Populate L2-compatible schema with proxy values unless real values already exist
    if "L2 Status" not in df.columns:
        df["L2 Status"] = l2_status
    else:
        mask = df["L2 Status"].isna() | (df["L2 Status"].astype(str).str.upper().isin(["", "UNAVAILABLE", "NO_DEPTH"]))
        df.loc[mask, "L2 Status"] = pd.Series(l2_status, index=df.index)[mask]

    if "L2 Quality" not in df.columns:
        df["L2 Quality"] = liquidity_quality
    else:
        mask = df["L2 Quality"].isna() | (df["L2 Quality"].astype(str).str.upper().isin(["", "THIN"]))
        df.loc[mask, "L2 Quality"] = pd.Series(liquidity_quality, index=df.index)[mask]

    if "L2 Imbalance" not in df.columns:
        df["L2 Imbalance"] = order_flow_imbalance
    else:
        df["L2 Imbalance"] = pd.to_numeric(df["L2 Imbalance"], errors="coerce").fillna(order_flow_imbalance)

    if "L2 Best Bid" not in df.columns:
        df["L2 Best Bid"] = best_bid_proxy
    else:
        df["L2 Best Bid"] = pd.to_numeric(df["L2 Best Bid"], errors="coerce").fillna(best_bid_proxy)

    if "L2 Best Ask" not in df.columns:
        df["L2 Best Ask"] = best_ask_proxy
    else:
        df["L2 Best Ask"] = pd.to_numeric(df["L2 Best Ask"], errors="coerce").fillna(best_ask_proxy)

    if "L2 Microprice" not in df.columns:
        df["L2 Microprice"] = microprice_proxy
    else:
        df["L2 Microprice"] = pd.to_numeric(df["L2 Microprice"], errors="coerce").fillna(microprice_proxy)

    if "BID_ASK_SPREAD_PCT" not in df.columns:
        df["BID_ASK_SPREAD_PCT"] = spread_proxy_pct
    else:
        df["BID_ASK_SPREAD_PCT"] = pd.to_numeric(df["BID_ASK_SPREAD_PCT"], errors="coerce").fillna(spread_proxy_pct)

    # Upgrade institutional score so the downstream ranker can use it immediately
    df["institutional_score"] = _clip01_series(0.30 * base_inst + 0.35 * institutional_flow_score + 0.20 * order_flow_score + 0.15 * absorption_score)
    return df




def _trend_label_fast(close: pd.Series, fast: pd.Series, slow: pd.Series, slope_window: int = 5) -> pd.Series:
    close = pd.to_numeric(close, errors="coerce")
    fast = pd.to_numeric(fast, errors="coerce")
    slow = pd.to_numeric(slow, errors="coerce")
    slope = fast.diff().rolling(slope_window, min_periods=max(2, slope_window // 2)).mean()
    spread = (fast / slow.replace(0, np.nan)) - 1.0

    out = pd.Series("SIDEWAYS", index=close.index, dtype="object")
    bull = (fast > slow) & (slope > 0)
    bear = (fast < slow) & (slope < 0)
    recovery = (close >= fast) & (slope > 0) & (spread > -0.02)
    weakening = (close <= fast) & (slope < 0) & (spread < 0.02)

    out.loc[bull | recovery] = "UP"
    out.loc[bear | weakening] = "DOWN"
    out.loc[(spread.abs() <= 0.01) & (slope.abs() <= close.pct_change().rolling(10, min_periods=5).std().fillna(0) * close.mean() * 0.02)] = "SIDEWAYS"
    return out
...
def compute_indicators(df: pd.DataFrame, symbol: str) -> pd.DataFrame:
    ...
    df["HTF_Trend"] = _trend_label_fast(df["close"], df["EMA_21"], df["EMA_50"], slope_window=7)
    df["ITF_Trend"] = _trend_label_fast(df["close"], ema8, df["EMA_21"], slope_window=5)
    df["LTF_Trend"] = _trend_label_fast(df["close"], ema5, ema13, slope_window=3)
    ...
    # stage-aware trend repair to avoid false DOWN labels during accumulation/base building
    acc_mask = df["market_stage"].astype(str).str.contains("Accumulation", case=False, na=False)
    mu_mask = df["market_stage"].astype(str).str.contains("Mark-Up", case=False, na=False)
    dist_mask = df["market_stage"].astype(str).str.contains("Distribution", case=False, na=False)
    md_mask = df["market_stage"].astype(str).str.contains("Mark-Down", case=False, na=False)

    df.loc[acc_mask & (pd.to_numeric(df.get("EMA_21_slope"), errors="coerce").fillna(0) >= -0.02), "HTF_Trend"] = "SIDEWAYS"
    df.loc[acc_mask & (pd.to_numeric(df.get("MACD_hist_slope"), errors="coerce").fillna(0) > 0), "ITF_Trend"] = "UP"
    df.loc[mu_mask, "HTF_Trend"] = "UP"
    df.loc[dist_mask & (df["HTF_Trend"] == "UP"), "HTF_Trend"] = "SIDEWAYS"
    df.loc[md_mask, "HTF_Trend"] = "DOWN"
    ...
    # L2 placeholders so scoring can apply consistent execution intelligence
    if "L2 Status" not in df.columns:
        df["L2 Status"] = "UNAVAILABLE"
    if "L2 Quality" not in df.columns:
        df["L2 Quality"] = "THIN"
    if "L2 Imbalance" not in df.columns:
        df["L2 Imbalance"] = np.nan
    if "L2 Best Bid" not in df.columns:
        df["L2 Best Bid"] = np.nan
    if "L2 Best Ask" not in df.columns:
        df["L2 Best Ask"] = np.nan
    if "L2 Microprice" not in df.columns:
        df["L2 Microprice"] = np.nan
    if "BID_ASK_SPREAD_PCT" not in df.columns:
        df["BID_ASK_SPREAD_PCT"] = np.nan
...
def analyze_symbol_all(symbol: str):
    try:
        ...
        result = {
            ...
            "Institutional Flow Score": round(float(pd.to_numeric(df.get("Institutional Flow Score"), errors="coerce").iloc[-1]), 2) if "Institutional Flow Score" in df.columns else 0.0,
            "Order Flow Score": round(float(pd.to_numeric(df.get("Order Flow Score"), errors="coerce").iloc[-1]), 2) if "Order Flow Score" in df.columns else 0.0,
