...
def candle_entries_multi(
    ...
    for w in weeks_list:
        ...
        # Force differentiated ladder across horizons so longer windows probe deeper support.
        spread_map = {2: 1.02, 4: 1.01, 6: 1.00, 8: 0.99, 12: 0.98, 18: 0.97, 30: 0.96}
        if np.isfinite(entry):
            entry = entry * spread_map.get(int(w), 1.0)
        ...
        if np.isfinite(floor_val):
            floor_factor = 1.0 - (0.01 * min(int(w), 20) / 20.0)
            floor_val = floor_val * floor_factor
        cap_val = last_close if np.isfinite(last_close) else np.nan

        if len(out) > 0:
            prev_w = sorted(out.keys())[-1]
            prev_entry = out.get(prev_w, np.nan)
            if np.isfinite(prev_entry) and np.isfinite(entry) and int(w) > int(prev_w):
                # Longer-horizon entries should not be shallower than earlier ones.
                entry = min(entry, prev_entry * 0.999)

        entry = _clamp(entry, floor_val, cap_val)
        out[int(w)] = round(entry, 4) if np.isfinite(entry) else np.nan

    # Detect pathological flat ladders and gently spread horizons deeper.
    vals = [round(v, 2) for v in out.values() if np.isfinite(v)]
    if len(set(vals)) <= 1 and vals:
        adjusted = {}
        for w2, v in out.items():
            if np.isfinite(v):
                adjusted[w2] = round(v * (1 - 0.002 * int(w2)), 4)
            else:
                adjusted[w2] = v
        out = adjusted
...
def compute_entry_prices(
    ...
    # -----------------------------
    if stage in {"distribution", "markdown"}:
        # Defensive stages should look deeper, but not force every name to the same bucket.
        deeper_candidate = e8 if np.isfinite(e8) else refined_buy
        if np.isfinite(p) and p < 0.35 and np.isfinite(e12):
            deeper_candidate = e12
        refined_buy = deeper_candidate
        ml_mode = "defensive"
        ml_target = "8w" if not (np.isfinite(p) and p < 0.35) else "12w+"
        ml_bias -= 0.02
