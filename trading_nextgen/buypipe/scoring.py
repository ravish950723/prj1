...
def _derive_best_risk_reward(row: Dict[str, Any]) -> float:
    candidates = []
    ...
    for key in (
        ...
        val = safe_float(row.get(key), np.nan)
        if np.isfinite(val) and val > 0:
            candidates.append(float(val))

    risk_pct = safe_float(row.get("Risk_%"), np.nan)
    reward_pct = safe_float(row.get("Reward_%"), np.nan)
    if np.isfinite(risk_pct) and risk_pct > 0 and np.isfinite(reward_pct) and reward_pct > 0:
        candidates.append(float(reward_pct / risk_pct))
...
def _entry_quality_score(
    ...
    rr_component = 0.0
    if np.isfinite(best_risk_reward):
        if best_risk_reward < 1.20:
            rr_component = -0.20
        elif best_risk_reward < 1.50:
            rr_component = -0.08
        elif best_risk_reward >= 3.00:
            rr_component = 0.15
        elif best_risk_reward >= 2.00:
            rr_component = 0.10
        else:
            rr_component = 0.05
...
def _v4_composite_score(
    *,
    ...
    flow_component = (
            0.06 * np.clip(order_flow_score, 0.0, 1.0) +
            0.05 * np.clip(institutional_flow_score, 0.0, 1.0) +
            0.04 * np.clip(absorption_score, 0.0, 1.0)
    )

    score = (
            0.32 * model_probability +
            0.18 * entry_quality_score +
            0.11 * signal_score +
            0.10 * institutional_score +
            0.07 * mtf_alignment_score +
            0.08 * substage_confidence +
            0.10 * regime_quality_score +
            flow_component
    )
    score += float(substage_decision_bias)
    return float(np.clip(score, 0.0, 1.0))
...
def final_recommendation(
    ...
    if np.isfinite(best_risk_reward) and best_risk_reward < 1.20:
        return "AVOID", 0.0, "low_risk_reward"

    if macro_warn:
        composite = min(composite, 0.74)
    ...
    if np.isfinite(best_risk_reward) and best_risk_reward >= 2.5:
        composite *= 1.08
    elif np.isfinite(best_risk_reward) and best_risk_reward < 1.5:
        composite *= 0.92
...
def apply_scoring(
    ...
    best_risk_reward = _derive_best_risk_reward(row)
    ...
    row["Best_Risk_Reward"] = _derive_best_risk_reward(row)
    rr_value = safe_float(row.get("Best_Risk_Reward"), np.nan)
    row["RR_Quality"] = (
        "EXCELLENT" if np.isfinite(rr_value) and rr_value >= 3.0 else
        "GOOD" if np.isfinite(rr_value) and rr_value >= 2.0 else
        "AVERAGE" if np.isfinite(rr_value) and rr_value >= 1.5 else
        "POOR" if np.isfinite(rr_value) else np.nan
    )
