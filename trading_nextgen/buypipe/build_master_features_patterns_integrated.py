...
from .compute import compute_mean_reversion, compute_risk_reward, generate_exit_reason
...
from .upward import detect_mean_reversion_buy
from .institutional_investor import score_institutional_investor
from .patterns_engine import evaluate_patterns
...
def _compute_pattern_pack(out: dict) -> dict:
    ...
    if _safe_bool(out.get("Darvas Breakout Active")) or _safe_str(out.get("Darvas Signal")).upper() == "Y":
        flags.append("DARVAS_BREAKOUT")
    if _safe_str(out.get("SMC Structure State")).upper() == "BULLISH_BREAK" or _safe_str(
            out.get("SMC_Breakout")).upper() == "Y":
        flags.append("SMC_BREAKOUT")
    if _safe_str(out.get("Mean Reversion State")).upper() in {"DEEP_UNDERVALUE", "UNDER_VWAP"} or _safe_str(
            out.get("Mean_Reversion")).upper() == "Y":
        flags.append("MEAN_REVERSION")
    if _safe_bool(out.get("Bullish_Engulfing")) or _safe_str(out.get("Bullish_Engulfing")).upper() == "Y":
        flags.append("BULLISH_ENGULFING")
    if _safe_bool(out.get("Hammer")) or _safe_str(out.get("Hammer")).upper() == "Y":
        flags.append("HAMMER")

    primary = flags[0] if flags else "NONE"
    return {
        "Patterns": ", ".join(flags) if flags else "NONE",
...
def _compute_entry_execution_pack(out: dict) -> dict:
    ...
    return {
        "ML Entry Target": buy,
        "ML Entry Mode": mode,
        "ML Entry Bias ATR": float((current - buy) / atr) if np.isfinite(atr) and atr != 0 else np.nan,
...
def backfill_output_columns(symbol: str, df: pd.DataFrame, out: dict, qqq_df: Optional[pd.DataFrame] = None) -> dict:
    ...
    out.update(_compute_pattern_pack(df, out))
    ...
    if not _safe_str(out.get("Pattern Detected")):
        out["Pattern Detected"] = _safe_str(out.get("Primary Pattern"), "NONE")
...
def compute_symbol_features(
    ...
    try:
        ...
        vals = [v for v in [rr1, rr2] if np.isfinite(v)]

        if len(vals) >= 2:
            rr = np.percentile(vals, 75)
        elif len(vals) == 1:
            rr = vals[0]
        else:
            ...
            if np.isfinite(entry) and np.isfinite(atr) and atr > 0:
                ...
                rr = compute_risk_reward(entry, target, stop)
            else:
                rr = np.nan

        out["Best_Risk_Reward"] = rr
