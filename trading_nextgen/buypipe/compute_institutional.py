...
def compute_indicators(df: pd.DataFrame, symbol: str) -> pd.DataFrame:
    ...
    # 🔥 institutional-grade confidence compression / generic-label penalty
    df["substage_confidence"] = np.clip(
        0.60 * df["substage_confidence"]
        + 0.40 * (pd.to_numeric(df["ADX_14"], errors="coerce").fillna(0.0) / 50.0).clip(0.0, 1.0),
        0.20,
        0.85,
    )
    ...
    df.loc[generic_mask, "substage_confidence"] = (
        pd.to_numeric(df.loc[generic_mask, "substage_confidence"], errors="coerce").fillna(0.0) * 0.85
    ).clip(0.20, 0.80)
...
def _classify_market_substage(
    ...
    """
    Institutional-grade substage classifier.

    Design goals:
      - prefer specific structural/tactical states over generic buckets
      - avoid over-dominance of BASE_FORMATION / CONTINUATION_DOWNTREND
      - keep confidence informative rather than always near 1.0
      - assign exactly one valid label per row, aligned to substages.yml
    """
    idx = df.index
    n = len(df)
    ...
    adx = _safe_series(df, "ADX_14", default=0.0).fillna(0.0)
    rsi = _safe_series(df, "RSI_14", default=50.0).fillna(50.0)
    ema20 = _safe_series(df, "EMA_20")
    ema21 = _safe_series(df, "EMA_21")
    ema50 = _safe_series(df, "EMA_50")
    ema200 = _safe_series(df, "EMA_200")
    macd_hist = _safe_series(df, "MACD_hist", default=0.0).fillna(0.0)
    macd_hist_slope = _safe_series(df, "MACD_hist_slope", default=0.0).fillna(0.0)
    vwap_support = _safe_series(df, "vwap_support")
    volume_avg_20 = _safe_series(df, "volume_avg_20")
    ...
    institution = _safe_series(df, "institutional_score", default=0.0).fillna(0.0)
    volume_weight = _safe_series(df, "volume_weight", default=0.0).fillna(0.0)
    ...
    close_location = ((close - low) / (price_range + 1e-9)).clip(0.0, 1.0).fillna(0.5)

    vol_ratio = (
        (volume / volume_avg_20.replace(0, np.nan))
        .replace([np.inf, -np.inf], np.nan)
        .fillna(0.0)
    )
    ret_3 = close.pct_change(3).fillna(0.0)
    ret_5 = close.pct_change(5).fillna(0.0)
    ret_10 = close.pct_change(10).fillna(0.0)
    ...
    near_20d_low = ((close - rolling_20_low).abs() / close.replace(0, np.nan)).fillna(1.0) <= 0.03
    near_20d_high = ((rolling_20_high - close).abs() / close.replace(0, np.nan)).fillna(1.0) <= 0.03
    near_60d_low = ((close - rolling_60_low).abs() / close.replace(0, np.nan)).fillna(1.0) <= 0.05
    near_60d_high = ((rolling_60_high - close).abs() / close.replace(0, np.nan)).fillna(1.0) <= 0.05
    ...
    ema21_slope = ema21.diff(5).fillna(0.0)
    ema50_slope = ema50.diff(10).fillna(0.0)

    hammer_like = (lower_wick > 2.0 * body) & (upper_wick <= body * 1.2)
    shooting_star = (upper_wick > 2.0 * body) & (lower_wick <= body * 1.2)

    score = pd.DataFrame(0.0, index=idx, columns=sorted(VALID_SUBSTAGES))
    ...
    # base generic priors: intentionally weak
    add_score(stage.eq("ACCUMULATION"), "BASE_FORMATION", 0.45)
    add_score(stage.eq("MARKUP"), "TREND_CONTINUATION", 0.45)
    add_score(stage.eq("DISTRIBUTION"), "RANGE_BOUND_DISTRIBUTION", 0.45)
    add_score(stage.eq("MARKDOWN"), "CONTINUATION_DOWNTREND", 0.45)
    ...
    capitulation = acc & (ret_5 <= -0.12) & (vol_ratio >= 1.9) & (rsi <= 32)
    selling_climax = acc & (ret_5 <= -0.08) & (vol_ratio >= 1.5) & near_20d_low
    automatic_rally = acc & (ret_3 >= 0.04) & (ret_10 > 0) & (close > ema20) & (macd_hist > 0)
    secondary_test_acc = acc & near_20d_low & (vol_ratio <= 1.05) & ret_5.between(-0.03, 0.02)
    spring = acc & (low < rolling_20_low.shift(1)) & (close > rolling_20_low.shift(1)) & hammer_like
    test_of_spring = acc & near_20d_low & (vol_ratio < 0.95) & (close > open_) & (ret_3 >= -0.01)
    low_vol_comp = acc & tight_range & (vol_ratio < 0.9) & ret_10.between(-0.04, 0.04)
    range_acc = acc & tight_range & close.between(rolling_20_low * 1.01, rolling_20_high * 0.99)
    accumulation_breakout = acc & breakout & (vol_ratio >= 1.25) & (close > ema20) & (macd_hist > 0)

    add_score(range_acc, "RANGE_BOUND_ACCUMULATION", 3.2)
    add_score(low_vol_comp, "LOW_VOLATILITY_COMPRESSION", 4.0)
    add_score(secondary_test_acc, "SECONDARY_TEST (ST_ACC)", 5.0)
    add_score(automatic_rally, "AUTOMATIC_RALLY (AR)", 5.5)
    add_score(selling_climax, "SELLING_CLIMAX (SC)", 6.8)
    add_score(capitulation, "CAPITULATION", 8.2)
    add_score(spring, "SPRING (SHAKEOUT)", 8.5)
    add_score(test_of_spring, "TEST_OF_SPRING", 7.2)
    ...
    breakout_confirmation = mu & breakout & (vol_ratio >= 1.15) & (close_location >= 0.65)
    bull_stack = mu & stacked_bull
    momentum_expansion = mu & (adx >= 24) & (macd_hist > 0) & (macd_hist_slope > 0) & (ret_10 > 0.05)
    trend_cont = mu & stacked_bull & (ret_20 > 0.03) & vol_ratio.between(0.85, 1.5)
    pullback_support = mu & stacked_bull & near_support & (ret_5 < 0) & (close >= ema50)
    vwap_reclaim = mu & (close > vwap_support) & (close.shift(1) <= vwap_support.shift(1)) & (vol_ratio >= 1.0)
    hv_breakout = mu & breakout & (vol_ratio >= 1.7)
    accel_phase = mu & (ret_10 >= 0.10) & (adx >= 28)
    climax_run = mu & (ret_5 >= 0.11) & (rsi >= 78) & (vol_ratio >= 1.9)

    add_score(early_trend, "EARLY_TREND_INITIATION", 4.0)
    add_score(breakout_confirmation, "BREAKOUT_CONFIRMATION", 6.5)
    add_score(bull_stack, "EMA_STACK_FORMATION (BULL_STACKED)", 5.0)
    add_score(momentum_expansion, "MOMENTUM_EXPANSION", 7.2)
    add_score(trend_cont, "TREND_CONTINUATION", 5.5)
    add_score(pullback_support, "PULLBACK_TO_SUPPORT (SMART_MONEY_ENTRY)", 8.2)
    add_score(vwap_reclaim, "VWAP_RECLAIM", 7.0)
    add_score(hv_breakout, "HIGH_VOLUME_BREAKOUT (CTA_BREAKOUT)", 9.0)
    add_score(accel_phase, "ACCELERATION_PHASE (PARABOLIC_MOVE)", 8.3)
    add_score(climax_run, "CLIMAX_RUN (BUYING_FRENZY)", 8.8)
    ...
    buying_climax = dist & (ret_5 >= 0.10) & (rsi >= 74) & (vol_ratio >= 1.7)
    upthrust = dist & (high > rolling_20_high.shift(1)) & (close < rolling_20_high.shift(1)) & shooting_star
    utad = dist & upthrust & near_60d_high & (vol_ratio >= 1.35)
    secondary_test_dist = dist & near_20d_high & (vol_ratio <= 1.05) & ret_5.between(-0.03, 0.03)
    lower_high = dist & (high < high.shift(10)) & (close < rolling_20_high) & (ema21_slope <= 0)
    failed_breakout = dist & breakout.shift(1).fillna(False) & (close < rolling_20_high.shift(1))
    distribution_breakdown = dist & breakdown & (vol_ratio >= 1.15)

    add_score(secondary_test_dist, "SECONDARY_TEST (ST_DIST)", 5.0)
    add_score(buying_climax, "BUYING_CLIMAX (BC)", 7.8)
    add_score(upthrust, "UPTHRUST (UT)", 7.0)
    add_score(utad, "UPTHRUST_AFTER_DISTRIBUTION (UTAD)", 8.7)
    ...
    momentum_selloff = md & (adx >= 24) & (macd_hist < 0) & (macd_hist_slope < 0)
    dead_cat = md & (ret_5 >= 0.04) & (ret_20 < 0) & (close < ema50)
    cont_down = md & stacked_bear & (ret_20 < -0.04)
    lower_low_acc = md & (low < rolling_20_low.shift(1)) & (vol_ratio >= 1.35)
    panic_sell = md & (ret_5 <= -0.12) & (vol_ratio >= 1.9) & (rsi <= 30)
    exhaustion_bottom = md & near_20d_low & hammer_like & (vol_ratio >= 1.1)

    add_score(reversal_init, "TREND_REVERSAL_INITIATION (BEARISH_BREAKDOWN)", 6.2)
    add_score(bear_stack, "EMA_STACK_FORMATION (BEAR_STACKED)", 5.0)
    add_score(momentum_selloff, "MOMENTUM_SELLOFF", 7.4)
    add_score(dead_cat, "DEAD_CAT_BOUNCE", 6.0)
    add_score(cont_down, "CONTINUATION_DOWNTREND", 5.2)
    add_score(lower_low_acc, "LOWER_LOW_ACCELERATION", 7.2)
    add_score(panic_sell, "PANIC_SELLING", 8.9)
    add_score(exhaustion_bottom, "EXHAUSTION_BOTTOMING", 7.0)

    # tactical boosts that create more specific institutional labeling
    add_score(acc & hammer_like & near_support & (institution >= 0.55), "SPRING (SHAKEOUT)", 1.8)
    add_score(acc & tight_range & (institution >= 0.55) & (vol_ratio < 1.0), "LOW_VOLATILITY_COMPRESSION", 1.0)
    add_score(mu & near_support & (institution >= 0.55), "PULLBACK_TO_SUPPORT (SMART_MONEY_ENTRY)", 1.5)
    add_score(mu & breakout & (institution >= 0.60), "HIGH_VOLUME_BREAKOUT (CTA_BREAKOUT)", 1.2)
    add_score(dist & shooting_star & near_20d_high, "UPTHRUST (UT)", 1.4)
    add_score(md & (ret_3 <= -0.07) & (vol_ratio >= 1.4), "LOWER_LOW_ACCELERATION", 1.1)
    add_score(md & hammer_like & near_20d_low & (institution >= 0.50), "EXHAUSTION_BOTTOMING", 1.4)

    # mild penalties for generic buckets when more specific evidence exists
    generic_acc_penalty = (score["BASE_FORMATION"] > 0) & (
        selling_climax | capitulation | spring | test_of_spring | accumulation_breakout | low_vol_comp
    )
    generic_md_penalty = (score["CONTINUATION_DOWNTREND"] > 0) & (
        momentum_selloff | lower_low_acc | panic_sell | exhaustion_bottom | reversal_init
    )
    score.loc[generic_acc_penalty, "BASE_FORMATION"] *= 0.60
    score.loc[generic_md_penalty, "CONTINUATION_DOWNTREND"] *= 0.55

    # choose label and calibrated confidence
    best_label = score.idxmax(axis=1)
    best_score = score.max(axis=1)
    second_score = score.apply(lambda r: float(r.nlargest(2).iloc[-1]) if (r > 0).sum() >= 2 else 0.0, axis=1)
    score_sum = score.sum(axis=1)

    fallback = pd.Series("NEUTRAL_RANGE", index=idx, dtype="object")
    out = best_label.where(best_score > 0, fallback).apply(_safe_substage_label)

    margin = (best_score - second_score).clip(lower=0.0)
    structural_quality = (
        0.45 * (adx / 35.0).clip(0.0, 1.0)
        + 0.25 * vol_ratio.clip(0.0, 2.0) / 2.0
        + 0.15 * institution.clip(0.0, 1.0)
        + 0.15 * volume_weight.clip(0.0, 2.0) / 2.0
    ).clip(0.0, 1.0)
    ...
    generic_labels = {
        ...
        "CONTINUATION_DOWNTREND",
        "TREND_CONTINUATION",
        "RANGE_BOUND_DISTRIBUTION",
        "RANGE_BOUND_ACCUMULATION",
    }
    generic_penalty = out.isin(generic_labels).astype(float) * 0.08

    substage_confidence = (
        0.42 * (best_score / score_sum.replace(0, np.nan)).fillna(0.0)
        + 0.33 * (margin / (best_score.replace(0, np.nan))).replace([np.inf, -np.inf], np.nan).fillna(0.0)
