...
from rl_policy import RLThresholdPolicy
...
BUY_THRESH = float(DEFAULT_BUY_PROB)
STRONG_BUY_PROB = 0.65
TECH_BUY_FALLBACK = 0.70


def get_confidence_band(prob: float) -> str:
    """Bucket a probability into a human-friendly confidence band.

    Keeps legacy behavior stable even if model calibration changes.
    """
    try:
        p = float(prob)
    except Exception:
...
RL_POLICY_FILE = HERE / "rl_threshold_policy.pkl"
_RL_POLICY = None
...
def _get_rl_policy():
    ...
    try:
        if RL_POLICY_FILE.exists():
            _RL_POLICY = RLThresholdPolicy.load(str(RL_POLICY_FILE))
        else:
            _RL_POLICY = False
    except Exception:
        _RL_POLICY = False
...
def _rl_thresholds_for_last_row(df: pd.DataFrame, prob: float) -> tuple[float, float]:
    """
    Returns (buy_threshold, strong_buy_threshold).
    If RL policy is unavailable, falls back to static thresholds.
    """
    policy = _get_rl_policy()
    if not policy:
        return float(BUY_THRESH), float(STRONG_BUY_THRESH)

    last = df.iloc[-1] if df is not None and len(df) else None
    state = {}
    for k in policy.feature_order:
        if k == "MODEL_PROBA":
            state[k] = float(prob)
            continue
        if last is not None and k in df.columns:
            try:
                state[k] = float(last.get(k))
            except Exception:
                state[k] = 0.0
        else:
            state[k] = 0.0
...
def _compute_indicator_snap(df):
    ...
    Never raises — returns dict with safe defaults.
    """
    try:
        ...
        # core numeric indicators (if already computed in df)
        for k in ["RSI14", "ADX14", "MACD", "MACD_SIGNAL", "VWAP", "ATR14_PCT",
                  "EMA21", "EMA50", "EMA200", "VWAP_DISTANCE_PCT", "VOL_SURGE_RATIO"]:
            if k in df.columns:
                v = last.get(k)
                try:
                    snap[k] = float(v) if v is not None and not pd.isna(v) else None
                except Exception:
                    snap[k] = None

        # EMA uptrend: EMA21 > EMA50 > EMA200 OR close above EMA21 and EMA21 rising
        ema21 = last.get("EMA21") if "EMA21" in df.columns else None
        ema50 = last.get("EMA50") if "EMA50" in df.columns else None
        ema200 = last.get("EMA200") if "EMA200" in df.columns else None
        close = last.get("close") if "close" in df.columns else last.get("CLOSE")
        ...
        ema21f, ema50f, ema200f, closef = map(_f, [ema21, ema50, ema200, close])
        ...
        snap["EMA21_Slope"] = float(ema21_slope) if np.isfinite(ema21_slope) else None

        # ADX strength label if ADX14 available
        adx = _f(last.get("ADX14")) if "ADX14" in df.columns else float("nan")
        if np.isfinite(adx):
            if adx >= 25:
                adx_strength = "STRONG"
            elif adx >= 18:
        ...
        ema_stack = np.isfinite(ema21f) and np.isfinite(ema50f) and np.isfinite(ema200f) and (ema21f > ema50f > ema200f)
        close_above = np.isfinite(closef) and np.isfinite(ema21f) and closef >= ema21f
        ema_rising = np.isfinite(ema21_slope) and ema21_slope > 0
...
def _pattern_list(rd: dict) -> str:
    """Return a comma-separated list of detected chart/candle patterns for a row dict."""
    # Keys are the columns your BUY engine may output. We check multiple aliases for robustness.
    candidates = [
        # Bases / bottoms / tops
        (['Triple Bottom', 'TRIPLE_BOTTOM', 'triple_bottom'], 'Triple Bottom'),
        (['Double Bottom', 'DOUBLE_BOTTOM', 'double_bottom'], 'Double Bottom'),
        (['Double Top', 'DOUBLE_TOP', 'double_top'], 'Double Top'),
        (['Head and Shoulders', 'HEAD_AND_SHOULDERS', 'head_and_shoulders'], 'Head and Shoulders'),
        (['Inverse Head and Shoulders', 'INVERSE_HEAD_AND_SHOULDERS', 'inverse_head_and_shoulders'], 'Inverse H&S'),
        # Continuations / breakouts
        (['Cup and Handle', 'CUP_AND_HANDLE', 'cup_and_handle'], 'Cup & Handle'),
        (['Bull Flag', 'BULL_FLAG', 'bull_flag'], 'Bull Flag'),
        (['Bear Flag', 'BEAR_FLAG', 'bear_flag'], 'Bear Flag'),
        (['Ascending Triangle', 'ASC_TRIANGLE', 'ascending_triangle'], 'Ascending Triangle'),
        (['Descending Triangle', 'DESC_TRIANGLE', 'descending_triangle'], 'Descending Triangle'),
        (['Sym Triangle', 'SYMMETRICAL_TRIANGLE', 'sym_triangle'], 'Sym Triangle'),
        (['Breakout', 'BREAKOUT_PATTERN'], 'Breakout'),
        # Candlesticks
        (['Bullish_Engulfing', 'Bullish Engulfing', 'BULLISH_ENGULFING'], 'Bullish Engulfing'),
        (['Bearish_Engulfing', 'Bearish Engulfing', 'BEARISH_ENGULFING'], 'Bearish Engulfing'),
        (['Hammer', 'HAMMER'], 'Hammer'),
        (['Shooting Star', 'SHOOTING_STAR', 'shooting_star'], 'Shooting Star'),
        (['Doji', 'DOJI'], 'Doji'),
        (['Morning Star', 'MORNING_STAR', 'morning_star'], 'Morning Star'),
        (['Evening Star', 'EVENING_STAR', 'evening_star'], 'Evening Star'),
        # Volatility / bands
        (['BB_Squeeze', 'Bollinger Squeeze', 'BB Squeeze'], 'BB Squeeze'),
        # Momentum signals (kept as 'patterns' for visibility)
        (['MACD Cross', 'MACD_Crossover', 'MACD_Cross'], 'MACD Cross'),
        (['RSI Divergence', 'RSI_DIVERGENCE', 'rsi_divergence'], 'RSI Divergence'),
    ]
    found = []
    for keys, label in candidates:
        ...
        if val is None:
            continue
        # treat numeric>0, boolean True, or 'yes'/'✅' as true
        if isinstance(val, (int, float)) and not (val is None) and (val == val) and val > 0:
            found.append(label)
            continue
...
# -----------------------------
# Model (lazy) + probability helper
# -----------------------------
_MODEL = None

def _get_model():
    """Lazy-load the strong_buy model (calibrated preferred, fallback to raw)."""
    global _MODEL
    if _MODEL is not None:
        return _MODEL
    # Prefer calibrated model if available
    for fname in ["strong_buy_xgb_model_calibrated.pkl", "strong_buy_xgb_model.pkl"]:
        p = HERE / fname
        if p.exists():
            try:
                ...
                print(f"⚠️ Failed to load model {p.name}: {e}")
    _MODEL = False
    print("⚠️ No model loaded; Model Probability will be 0.")
    return _MODEL

def _predict_proba_for_last_row(symbol: str, df: pd.DataFrame) -> float:
    """Return P(strong_buy)=1 for the last row of df using REQUIRED_FEATURES contract."""
    model = _get_model()
    if not model or REQUIRED_FEATURES is None:
        return 0.0

    if df is None or df.empty:
        return 0.0

    # Build a single-row feature frame aligned to the training contract
    last = df.iloc[[-1]].copy()
    ...
    try:
        ...
        if proba is None or (isinstance(proba, float) and (np.isnan(proba) or np.isinf(proba))):
            return 0.0
        return float(proba)
    except Exception as e:
        print(f"⚠️ {symbol}: predict_proba failed ({e})")
        return 0.0
...
def fill_missing_output_columns(df_summary: pd.DataFrame) -> pd.DataFrame:
    ...
    # Sentiment columns
    # -------------------------
    if "Sentiment Confidence" not in out.columns:
        out["Sentiment Confidence"] = None
...
def add_excel_derived_columns(df):
    ...
    try:
        ...
        # ------------------------------------------------------------
        # Derived flags used by the Excel / ranking layer
        # ------------------------------------------------------------
        breakout = []
        underval = []
        reversal = []
        ...
        for _, r in df.iterrows():
            ...
            # Undervalued heuristic (if you don't have a direct 'Current Price' in summary)
            ...
            macd_cross = _is_yes(rd.get('MACD Cross')) or _is_yes(rd.get('MACD_Crossover'))
            rsi_state = str(rd.get('RSI State') or '').lower()
            rev = (
                _is_yes(rd.get('Price Reversal'))
                or _is_yes(rd.get('Bullish_Engulfing'))
                or _is_yes(rd.get('Bullish Engulfing'))
            ...
            # DipReclaim: near support + reversal + (trend up / strong trend strength) + volume/obv confirmation
        ...
        df['Undervalued'] = underval
        df['Trend Reversal'] = reversal
        df['Pattern Detected'] = patterns
        df['DipReclaim'] = dipreclaim
...
def main():
    ...
    for res in summary:
        ...
        try:
            ...
            buy_thr, strong_thr = _rl_thresholds_for_last_row(df, prob)
            res["RL Buy Threshold"] = round(float(buy_thr), 2)
            res["RL Strong Threshold"] = round(float(strong_thr), 2)
            res["Model-Driven Buy"] = "✅" if prob >= buy_thr else "❌"
            res["Model-Driven Strong Buy"] = "✅" if prob >= strong_thr else "❌"
